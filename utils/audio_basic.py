...
def _extract_wav_pcm(payload: bytes) -> Tuple[bytes, int, int, int]:
    ...
    if len(payload) < 44 or payload[0:4] != b'RIFF' or payload[8:12] != b'WAVE':
        raise BasicAudioError("Unexpected WAV header from ElevenLabs")
...
def _tts_turn(text: str, voice_id: str, api_key: str, format: str = 'wav') -> bytes:
    headers = {
        'xi-api-key': api_key,
        'accept': 'audio/wav' if format == 'wav' else 'audio/mpeg',
    ...
    r = requests.post(ELEVEN_API_TTS.format(voice_id=voice_id), headers=headers, json=json_payload, timeout=60)
    if r.status_code != 200:
        raise BasicAudioError(f"ElevenLabs TTS failed ({r.status_code}): {r.text[:120]}")
    return r.content
...
def synthesize_episode_basic(
    ...
    silence_frames = None  # computed after first segment
    ...
    for idx, turn in enumerate(script, 1):
        ...
        wav_bytes = _tts_turn(text, voice_id, eleven_key, format='wav')
        pcm, srate, ch, bits = _extract_wav_pcm(wav_bytes)

        if sr is None:
            sr, channels, bps = srate, ch, bits
            # compute silence chunk
        ...
        # Append silence after each turn except last
        if idx != total_turns:
